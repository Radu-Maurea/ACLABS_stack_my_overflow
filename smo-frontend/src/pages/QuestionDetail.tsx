      ...
      <div className="max-w-5xl mx-auto px-10 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-xs text-gray-500 ml-auto">
            Asked by <span className="font-medium text-gray-700">{question.author?.username ?? 'Anonymous'}</span> · {formatDate(question.created_at)}
          </span>
        </div>
            ...
            <div key={a.id} className={`border rounded-lg p-6 shadow-sm ${a.is_accepted ? 'border-green-400 bg-green-50/30' : ''}`}>
              ...
              <div className="flex items-center gap-3 text-xs text-gray-500">
                ...
                <span className="ml-auto">
                  Answered by <span className="font-medium text-gray-700">{a.author?.username ?? 'Anonymous'}</span> · {formatDate(a.created_at)}
                </span>
              </div>
